<?php
include '../connection/connection.php'; // Include your database conn script

// Check if the form is submitted
if(isset($_POST['name'], $_POST['email'], $_POST['username'], $_POST['new_password'])) {
    // Retrieve form data
    $name = $_POST['name'];
    $email = $_POST['email'];
    ...
    if(empty($name) || empty($email) || empty($username) || empty($new_password)) {
        echo "Please fill in all fields.";
        exit;
    }

    // Check if the user exists in the database
    $query = "SELECT * FROM users WHERE name = '$name' AND email = '$email' AND username = '$username'";
